...
$mdpconnect=sha1($_POST['password']);
     ...
     if(!empty($id) AND !empty($mdpconnect))
    {
        ...
        $sq=$db->query("SELECT * FROM utilisateur WHERE id_user= '{$id}' AND mot_de_passe ='{$mdpconnect}' ");
        $row_cnt = $sq->num_rows;
                if($row_cnt==1){
                  $row = mysqli_fetch_array($sq);
                    if($row['type']==gr){

                           $sqll=$db->query("SELECT * FROM client WHERE id_client= '{$id}'");

                   $userinfo=$sqll->fetch_array(MYSQLI_BOTH);
                    ...
                    $_SESSION['wilaya']=$userinfo['wilaya'];
                    $_SESSION['telephone']=$userinfo['telephone'];
                    $_SESSION['society']=$userinfo['society'];
                    $_SESSION['mail']=$userinfo['mail'];
                    
                    header("Location: client/profil.php?id_client=".$_SESSION['id_client']);
                    
                    }else if ($row['type']==gs){
                        $sqll=$db->query("SELECT * FROM gestionnaire WHERE id= '{$id}'");
                        $userinfo=$sqll->fetch_array(MYSQLI_BOTH);

                        $_SESSION['id_gest']= $userinfo['id'];
                        
                        header("Location: gestionnaire/statistique.php?id_gestionnaire=".$_SESSION['id_gest']);


                    }else if($row['type']==ad){
                    $sqll=$db->query("SELECT * FROM administrateurs WHERE id= '{$id}'");
                        $userinfo=$sqll->fetch_array(MYSQLI_BOTH);

                        $_SESSION['id_gest']= $userinfo['id'];
                        
                        header("Location: admin/statistique.php?id_gestionnaire=".$_SESSION['id_gest']);

                    
                    }
                
                }else{$erreur="mdp ou mail non valide"; }
    
    }else {$erreur2="tt les champs doivent etre complété"; }
...
<section style="background-color:#f2f4f3" id=classe>
        ...
        <div class="container-fluid">
                 <h1 style="margin-top:-110px; text-align:center;" class="section-heading"><span style="color:#004b88;"class="glyphicon glyphicon-triangle-right" ></span> <font style=" font-size: 40px;"> Les classes thérapeutiques </font><span style="color:#004b88;"class="glyphicon glyphicon-triangle-left" ></span></h1><br><br>
...
?>
  ...
  <!-- main part--><section style="background-color:#7ea6c6;"id="login">
<h3 style="margin-top:-100px;"align="center" class="section-heading"><span style="color:#004b88;"class="glyphicon glyphicon-triangle-right" style="width:50px;"></span> <font style=" color:white;font-size: 40px;"> Connectez-vous pour accéder à votre compte et commander </font><span style="color:#004b88;"class="glyphicon glyphicon-triangle-left" style="width:50px;"></span></h3> <br> 

<div class="modal-dialog">
    <div class="modal-conten">
        <div class="modal-header">
            <?php if(isset($erreur2)){ echo'<div class="alert alert-danger"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
  <strong>Erreur:</strong> Les deux champs doivent étre complétés.
</div>';}else if(isset($erreur)){ echo'<div class="alert alert-danger"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
  <strong>Erreur:</strong> Identificateur ou mot de passe incorrecte.
</div>';}?>
          <h1 style="color:#004b88;"class="text-center"><b>Bienvenue</b></h1>
        </div>
         <div class="modal-body">
             				<form method="POST" action="#login">

             <div class="form-group">
                 <input type="text" name ="id"class="form-control input-lg" placeholder="Identificateur"/>
             </div>
             ...
             <div class="form-group">
                 <input required =""type="submit" name="submit"class="btn btn-block btn-lg btn-primary" value="Connecter"/>
                 <span class="pull-right"><a href="inscrip/index.php"><b>S'inscrire</b></a></span><span><a href="#"><b>Mot de passe oublier?</b></a></span>
             </div>
                                </form>
         </div>
    </div>
...
if(isset($_POST['f']))
{
  ...
  if($_POST['message'] AND !empty($_POST['message'])){
      $msg=htmlspecialchars($_POST['message']);
      if($_POST['email'] AND !empty($_POST['email'])){
$mail=htmlspecialchars($_POST['email']);
    if($_POST['nom'] AND !empty($_POST['nom'])){
$nom=htmlspecialchars($_POST['nom']);
        if($_POST['obj'] AND !empty($_POST['obj'])){
$obj=htmlspecialchars($_POST['obj']);
       
        
    
    $sql="INSERT into contact (message,nom,email,date,objet) Values ('{$msg}','{$nom}','{$mail}','{$date}','{$obj}')";
...
<section id="contact">
        <div class="container">
            <div class="row">
                <div class="col-lg-12 text-center">
                    <h2 style="margin-top:-100px;"class="section-heading"><span style="color:#004b88;"class="glyphicon glyphicon-triangle-right" style="width:50px;"></span> <font style=" font-size: 50px;"> Contactez-nous </font><span style="color:#004b88;"class="glyphicon glyphicon-triangle-left" style="width:50px;"></span></h2>
               <?php   if(!$c){ echo'<div class="alert alert-danger"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
  <strong>Erreur:</strong> Veuillez remplir tous les champs.
</div>';}
if($b){ echo '<div class="alert alert-success">
  <strong>Message envoyé</strong><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
</div >'; }?>
            ...
            <div class="row">
                <div class="col-lg-12">
                    <form action="#contact"method="post"name="sentMessage" id="contactForm" novalidate>
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                         <input required="" maxlength="30" name="nom" type="text" class="form-control" placeholder="Votre nom *" id="name" >
                                    <p class="help-block text-danger"></p>
                                </div>
                                <div class="form-group">
                                    <input required="" maxlength="60"  name="email"  type="email" class="form-control" placeholder="Votre Email *" id="email" >
                                    <p class="help-block text-danger"></p>
                                </div>
                                <div class="form-group">
                                    <input  required="" maxlength="65"  type="text" name="obj" class="form-control" placeholder="Objet  *" id="phone">
                                    <p class="help-block text-danger"></p>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    <textarea  required="" class="form-control" name="message" placeholder="Votre message *" id="message"  ></textarea>
                                    <p class="help-block text-danger"></p>
                                </div>
                                                                <button type="submit" name ="f"class="btn btn-xl">Envoyer</button>
                                </form>
